...
import {
  ...
  proposalAcceptUrl,
  proposalRejectUrl,
  serviceRequestByIdUrl,
  serviceRequestCancelUrl,
  serviceRequestNextProposalUrl,
  serviceRequestProposalsUrl,
  serviceRequestVisibleProposalLimitUrl,
} from "../../lib/api";
...
function parseProposalsResponse(payload: unknown): ProposalsResponse | null {
  ...
  for (const item of root.items) {
    if (
      ...
      typeof item.professionalProfileId !== "string" ||
      !isNullableString(item.professionalName) ||
      typeof item.amountInCents !== "number" ||
      !Number.isInteger(item.amountInCents) ||
      item.amountInCents < 0 ||
      typeof item.estimatedDurationValue !== "number" ||
      !Number.isInteger(item.estimatedDurationValue) ||
      item.estimatedDurationValue < 1 ||
      !isProposalDurationUnit(item.estimatedDurationUnit) ||
      typeof item.message !== "string" ||
      !isProposalStatus(item.status) ||
...
function parseProposalAcceptanceResponse(
  payload: unknown,
): ParsedProposalAcceptanceResponse | null {
  if (!isRecord(payload)) {
    return null;
  }
  ...
  if (
    ...
    !isRecord(root.conversation) ||
    typeof root.contract.id !== "string" ||
    root.contract.status !== "ACCEPTED" ||
    typeof root.contract.agreedAmountInCents !== "number" ||
    !Number.isInteger(root.contract.agreedAmountInCents) ||
    root.contract.agreedAmountInCents < 0 ||
    typeof root.contract.acceptedAt !== "string" ||
    typeof root.conversation.id !== "string" ||
    typeof root.conversation.status !== "string"
  ) {
    return null;
  }
...
function parseServiceRequestDetails(
  payload: unknown,
  ...
  if (
    ...
    typeof root.visibleProposalLimit !== "number" ||
    ...
    typeof root.editableUntil !== "string" ||
  ...
  let contract: ServiceRequestDetails["contract"] = null;

  if (root.contract !== null) {
    if (
      !isRecord(root.contract) ||
      typeof root.contract.id !== "string" ||
    ...
    const review = parseReviewSummary(root.contract.review);
    const customerReview = parseReviewSummary(root.contract.customerReview);
    ...
    if (
      root.contract.status !== "ACCEPTED" &&
      root.contract.status !== "IN_PROGRESS" &&
      root.contract.status !== "COMPLETED" &&
      root.contract.status !== "CANCELLED"
    ) {
      return null;
    }
...
export function ServiceRequestDetailsPage({
  ...
  const [proposalsState, setProposalsState] =
    useState<ProposalsState>("idle");
  const [proposalsPage, setProposalsPage] = useState(1);

  const [proposalAcceptanceState, setProposalAcceptanceState] =
    useState<ProposalAcceptanceState>("idle");

  const [
    proposalAcceptanceProposalId,
    setProposalAcceptanceProposalId,
  ] = useState<string | null>(null);

  const [
    proposalAcceptanceResult,
    setProposalAcceptanceResult,
  ] = useState<ProposalAcceptanceResult | null>(null);

  const [proposalAcceptanceError, setProposalAcceptanceError] =
    useState<string | null>(null);

  const [editMessage, setEditMessage] = useState<string | null>(null);
  const [editMessageTone, setEditMessageTone] =
    useState<EditMessageTone>("success");
  ...
  const [cancelBlockedByServer, setCancelBlockedByServer] =
    useState(false);
  ...
  const canCancel =
    request?.status === "OPEN" && !cancelBlockedByServer;

  const proposalsTotalPages = Math.max(
    1,
  ...
  const isAcceptingProposal =
    proposalAcceptanceState === "submitting";
  ...
  const loadServiceRequest = useCallback(async (): Promise<void> => {
    ...
    try {
      const httpResponse = await fetch(
        serviceRequestByIdUrl(serviceRequestId),
        {
          headers: {
  ...
  useEffect(() => {
    ...
    setEditMessage(
      "Esta solicitação não pode mais ser alterada diretamente. Alterações posteriores precisarão de análise da Soravi.",
    );
  }, [canEdit, isEditing]);
    ...
    async function loadCategories(): Promise<void> {
      ...
      try {
        ...
        const parsedCategories =
          payload.filter(isServiceCategory);

        setCategories(parsedCategories);

        setCategoriesState(
          parsedCategories.length > 0 ? "success" : "empty",
        );
      } catch {
  ...
  const loadProposals = useCallback(
    async (targetPage: number): Promise<void> => {
      ...
      try {
        const url = new URL(
          serviceRequestProposalsUrl(serviceRequestId),
        );
        ...
        const parsedProposals =
          parseProposalsResponse(payload);
        ...
        setProposalsState(
          parsedProposals.items.length > 0
            ? "success"
            : "empty",
        );
      } catch {
  ...
  function openProposalAcceptance(proposalId: string): void {
    if (
      isAcceptingProposal ||
      proposalAcceptanceState === "confirming"
    ) {
      return;
    }
  ...
  async function applyProposalPolicy(
    url: string,
    method: "POST" | "PATCH",
    body?: object,
  ): Promise<void> {
  ...
  async function completeContract(): Promise<void> {
    ...
    try {
      const response = await fetch(
        contractCompleteUrl(request.contract.id),
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (!response.ok) {
        throw new Error("contract completion failed");
      }
  ...
  async function submitReview(): Promise<void> {
    ...
    try {
      const response = await fetch(
        contractReviewUrl(request.contract.id),
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  async function acceptProposal(): Promise<void> {
    ...
    try {
      const response = await fetch(
        proposalAcceptUrl(proposalAcceptanceProposalId),
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
        ...
        const message =
          code === "PROPOSAL_NOT_FOUND"
            ...
            : code === "PROPOSAL_NOT_ACTIVE"
              ...
              : code ===
                "SERVICE_REQUEST_NOT_ACCEPTING_PROPOSALS"
      ...
      const parsedResult =
        parseProposalAcceptanceResponse(payload);

      if (!parsedResult) {
        setProposalAcceptanceError(
          "A contratação foi processada, mas não foi possível atualizar a confirmação.",
        );
        setProposalAcceptanceState("error");
        return;
      }
      ...
      setProposals((currentProposals) =>
        currentProposals
          ? {
            ...currentProposals,
            items: currentProposals.items.map(
              (proposal) =>
  ...
  async function submitUpdate(
    ...
    try {
      const response = await fetch(
        serviceRequestByIdUrl(request.id),
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${accessToken}`,
          ...
          body: JSON.stringify({
            ...
            location: {
              ...
              addressNumber:
                data.location.addressNumber,
              addressComplement:
                data.location.addressComplement,
            },
          }),
        },
      );
      ...
      const updatedRequest =
        parseServiceRequestDetails(payload);

      if (!updatedRequest) {
        setEditMessageTone("error");
        setEditMessage(
          "As alterações foram enviadas, mas não foi possível atualizar os dados exibidos.",
        );
        return;
      }
  ...
  async function cancelServiceRequest(): Promise<void> {
    ...
    try {
      const response = await fetch(
        serviceRequestCancelUrl(request.id),
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (response.status === 409) {
        ...
        setEditMessageTone("error");
        setEditMessage(
          "Esta solicitação não pode mais ser cancelada diretamente.",
        );
        return;
      }
      ...
      if (!cancelledRequest) {
        setEditMessageTone("error");
        setEditMessage(
          "A solicitação foi cancelada, mas não foi possível atualizar os dados exibidos.",
        );
        return;
      }
  ...
  const status =
    serviceRequestStatusPresentation[request.status];
  ...
  const ownReview = contract?.review ?? null;

  const receivedCustomerReview =
    contract?.customerReview ?? null;

  const receivedCustomerReviewIsPublished =
    receivedCustomerReview?.publishedAt != null;
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        ...
        <header className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <p className="mt-6 border-t border-slate-200 pt-5 text-sm leading-6 text-slate-600">
            Esta solicitação já foi publicada e não pode
            ser alterada diretamente.
          </p>
          ...
          {canCancel &&
            ...
            <div className="mt-5 border-t border-slate-200 pt-5">
              <button
                ...
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-red-300 bg-white px-4 py-2.5 font-semibold text-red-700 hover:border-red-400 hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 focus-visible:ring-offset-2"
              >
                <CircleX
                  aria-hidden="true"
                  className="size-4"
        ...
        {editMessage ? (
          <div
            role={
              editMessageTone === "error"
                ? "alert"
                : "status"
            }
            className={`mt-5 flex items-start gap-3 rounded-xl border p-4 ${editMessageTone === "success"
        ...
        {isCancelConfirmationOpen ? (
          <section
            ...
            className="mt-5 rounded-2xl border border-red-200 bg-white p-5 shadow-sm sm:p-7"
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 leading-7 text-slate-600">
                  Ela não será enviada a novos profissionais.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <textarea
                ...
                placeholder="Conte brevemente por que deseja cancelar"
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <button
                ...
                className="inline-flex min-h-12 items-center justify-center px-5 py-3 font-semibold text-slate-700 hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Voltar
              </button>
              ...
              <button
                ...
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-red-600 px-6 py-3 font-semibold text-white hover:bg-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-red-300"
              >
                {isCancelling ? (
                  <Loader2
                    aria-hidden="true"
          ...
          >
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-600">
                    Atualize os dados antes do encerramento da
                    janela inicial.
                  </p>
                </div>
                ...
                <button
                  ...
                  className="inline-flex size-10 shrink-0 items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 disabled:opacity-50"
                >
                  <X
                    aria-hidden="true"
                    className="size-5"
                ...
                <div>
                  ...
                  {categoriesState === "empty" ? (
                    <p
                      ...
                      role="status"
                    >
                      Nenhuma categoria está disponível no
                      momento.
                    </p>
                  ) : null}
                </div>
              ...
              <div className="mt-6 grid gap-5 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    maxLength={32}
                    aria-invalid={Boolean(
                      errors.location?.addressNumber,
                    )}
                    aria-describedby={
                      errors.location?.addressNumber
                        ? "edit-addressNumber-error"
                        : undefined
                    }
                    className={fieldClassName}
                    {...register(
                ...
                <div>
                  <label
                    htmlFor="edit-addressComplement"
                    className="text-sm font-semibold text-slate-800"
                  >
                    Complemento{" "}
                    <span className="font-normal text-slate-500">
                  ...
                  <FieldError
                    id="edit-addressComplement-error"
                    message={
                      errors.location
                        ?.addressComplement?.message
                    }
                  />
                </div>
              </div>
            ...
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <button
                ...
                disabled={isSubmitting}
                onClick={() => setIsEditing(false)}
                className="inline-flex min-h-12 items-center justify-center px-5 py-3 font-semibold text-slate-700 hover:text-slate-950 disabled:opacity-50"
              >
                Cancelar
              </button>
              ...
              <button
                ...
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-3 font-semibold text-white hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-slate-400"
              >
                {isSubmitting ? (
                  <Loader2
                    aria-hidden="true"
        ...
        {!isEditing ? (
          <section
            aria-labelledby="received-proposals-title"
            className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"
            aria-live="polite"
          >
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Compare as condições enviadas para esta
                  solicitação.
                </p>
              </div>
              ...
              {proposalsState !== "idle" &&
                ...
                <p className="text-sm text-slate-600">
                  Página{" "}
                  {proposals?.pagination.page ??
                    proposalsPage}{" "}
                  de {proposalsTotalPages}
                </p>
              ) : null}
            </div>
            ...
            {proposalsState === "error" ||
              proposalsState === "unauthorized" ||
              proposalsState === "forbidden" ? (
              <div
              ...
              >
                <p className="text-sm font-medium leading-6 text-red-700">
                  {proposalsState === "unauthorized"
                    ? "Sua sessão expirou. Entre novamente para consultar as propostas."
                    : proposalsState === "forbidden"
                      ? "Sua conta não possui permissão para consultar estas propostas."
                      : "Não foi possível carregar as propostas. Tente novamente."}
                </p>
                ...
                <button
                  ...
                  className="mt-3 inline-flex min-h-11 items-center justify-center rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 focus-visible:ring-offset-2"
                >
                  Tentar novamente
                </button>
              </div>
            ...
            {proposalsState === "empty" ? (
              <p className="py-10 text-sm leading-6 text-slate-600">
                Ainda não há propostas para esta
                solicitação.
              </p>
            ) : null}

            {proposalAcceptanceState === "success" &&
              proposalAcceptanceResult ? (
              <div
              ...
              >
                ...
                <p className="mt-2 text-sm leading-6 text-emerald-800">
                  ...
                  {formatProposalAmount(
                    proposalAcceptanceResult.contract
                      .agreedAmountInCents,
                  )}
                  .
                </p>
            ...
            {request.conversationId ? (
              <Link
                href={`/conversas/${request.conversationId}`}
                className="mt-5 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-700 focus-visible:ring-offset-2"
              >
                <MessageCircle
                  aria-hidden="true"
                  className="size-4"
              ...
              >
                <p className="font-semibold text-blue-950">
                  {contract.status === "ACCEPTED"
                    ? "Aguardando início do serviço pelo profissional"
                ...
                {contractActionMessage ? (
                  <p
                    className={`mt-3 text-sm font-medium ${contractActionMessage ===
                        "Serviço concluído com sucesso."
                        ? "text-emerald-700"
                        : "text-red-700"
                      }`}
                ...
                {contract.status === "COMPLETED" &&
                  ...
                  !receivedCustomerReviewIsPublished ? (
                  <div className="mt-5 border-t border-blue-200 pt-4">
                    ...
                    <p className="mt-2 text-sm leading-6 text-blue-900">
                      Uma avaliação já foi enviada. Ela
                      ficará visível quando você também
                      avaliar ou quando o período de
                      avaliação terminar.
                    </p>
                  </div>
                ) : null}
                ...
                {contract.status === "COMPLETED" &&
                  ...
                  <div className="mt-5 border-t border-blue-200 pt-4">
                    ...
                    <p className="mt-2 text-sm">
                      Nota:{" "}
                      {receivedCustomerReview.rating} de 5
                    </p>

                    {receivedCustomerReview.comment ? (
                ...
                {contract.status === "COMPLETED" &&
                  ...
                  >
                    <h3 className="font-semibold text-blue-950">
                      Sua avaliação do profissional
                    </h3>

                    <p className="mt-2 text-sm">
                      Nota: {ownReview.rating} de 5
                    </p>
                ...
                {contract.status === "COMPLETED" &&
                  ...
                  <div className="mt-5 border-t border-blue-200 pt-4">
                    ...
                    <button
                      ...
                      disabled={isSubmittingReview}
                      className="mt-3 inline-flex min-h-11 rounded-xl bg-blue-600 px-4 py-2 font-semibold text-white disabled:opacity-60"
                    >
                      {isSubmittingReview
                        ? "Enviando..."
                        : "Enviar avaliação"}
                ...
                {contract.status === "COMPLETED" &&
                  ...
                  <div className="mt-5 border-t border-blue-200 pt-4">
                    <p className="font-medium text-slate-700">
                      O período para avaliar este serviço
                      foi encerrado.
                    </p>
                  </div>
                ) : null}
            ...
            {proposalAcceptanceState ===
              ...
              >
                ...
                <p className="mt-2 text-sm leading-6 text-blue-900">
                  Ao aceitar esta proposta, as demais
                  propostas ativas serão encerradas e o
                ...
                <div className="mt-4 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                  <button
                    type="button"
                    onClick={closeProposalAcceptance}
                    disabled={isAcceptingProposal}
                    className="inline-flex min-h-11 items-center justify-center rounded-xl border border-blue-300 bg-white px-4 py-2 text-sm font-semibold text-blue-800 hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Voltar
                  </button>
            ...
            {proposalsState === "success" &&
              proposals ? (
              <>
                {!request.contract ? <div className="mt-4 flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      void applyProposalPolicy(
                        serviceRequestNextProposalUrl(
                          request.id,
                        ),
                        "POST",
                ...
                <div className="mt-5 grid gap-4">
                  {proposals.items.map((proposal) => (
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-2xl font-bold text-slate-950">
                            {formatProposalAmount(
                              proposal.amountInCents,
                            )}
                          </p>
                        </div>

                        <span
                          className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${proposalStatusTones[proposal.status]}`}
                        >
                          {
                        ...
                        <div>
                          ...
                          <dd className="mt-1">
                            {formatProposalDuration(
                              proposal.estimatedDurationValue,
                              proposal.estimatedDurationUnit,
                            )}
                          </dd>
                        </div>
                      ...
                      {proposal.status === "ACTIVE" &&
                        ...
                        <div className="mt-4 flex flex-wrap gap-3">
                          ...
                          <button
                            ...
                            className="rounded-xl border border-red-200 px-4 text-sm font-semibold text-red-700"
                          >
                            Rejeitar
                          </button>
                        </div>
                      ...
                      <div className="mt-4">
                        <FavoriteProfessionalButton
                          professionalProfileId={
                            proposal.professionalProfileId
                          }
                        />
                      </div>
                    </article>
        ...
        {!isEditing ? (
          <section
            aria-labelledby="request-location-title"
            className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"
          >
            <div className="flex items-center gap-3">
              <MapPin
            ...
            <address className="mt-4 space-y-1 not-italic leading-7 text-slate-700">
              ...
              {location.addressComplement ? (
                <p>{location.addressComplement}</p>
              ) : null}
