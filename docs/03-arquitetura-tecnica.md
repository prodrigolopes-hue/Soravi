...
-   A troca de senha autenticada foi concluída nos commits `f8ea104 feat(users): permite alterar a propria senha` e `777732b feat(web): adiciona seguranca da conta`. O backend expõe `PATCH /api/v1/users/me/password`, exige senha atual e nova senha, preserva a sessão atual, revoga as demais sessões, invalida tokens pendentes de password reset, aplica a política central de senha e não exige telefone verificado. O frontend expõe `/conta/seguranca` com link `Conta` no header autenticado desktop/mobile, confirmação somente no frontend e tratamento sanitizado dos códigos públicos.
-   A moderação administrativa `ACTIVE`/`BLOCKED` foi concluída nos commits `fce91dc feat(admin): adiciona bloqueio seguro de contas` e `7f7b5dd feat(admin): adiciona moderacao de contas no painel`, sem afirmar deploy. O backend expõe `PATCH /api/v1/users/admin/:userId/status`, usa guards de access token e role ADMIN, bloqueia `User` com `SELECT ... FOR UPDATE` e mantém status e revogação de sessões na mesma transação. O painel oferece confirmação inline e atualização local do item para clientes/profissionais, no mobile e desktop. O commit `020fd3b test(auth): adiciona testes reais de concorrencia` passou a comprovar em PostgreSQL real as duas ordens possíveis entre login e bloqueio, com contenção observada por `pg_blocking_pids()`; os detalhes e as proteções do banco dedicado estão na [arquitetura canônica](ARCHITECTURE.md#testes-de-integração).
-   O commit `9443580 test(auth): cobre concorrencia entre login e reset de senha` ampliou a suíte PostgreSQL real para as duas ordens entre login e confirmação de password reset. A contenção é observada por `pg_backend_pid()` e `pg_blocking_pids()`; login vencedor cria uma sessão depois revogada pelo reset, enquanto reset vencedor altera o hash e faz o login com a senha antiga reverter sem persistir sessão ou `lastLoginAt`.
-   O commit `50b0277 test(auth): cobre limite de sessoes sob logins concorrentes` comprovou com PostgreSQL real que dois logins para a mesma conta respeitam o máximo de cinco sessões: B espera pelo lock de A, lê o estado após o primeiro commit e reaplica a regra. Das sete sessões finais, somente as duas mais antigas por `createdAt ASC` + `id ASC` ficam revogadas; S3 vence o desempate de IDs e permanece ativa com S4, S5 e as duas novas. Login versus troca de senha autenticada e demais concorrências não implementadas permanecem pendentes.
