...
- `script-src` não usa mais `'unsafe-inline'`: usa nonce e `'strict-dynamic'`. `'unsafe-eval'` é incluído somente em development por compatibilidade com Next.js/Fast Refresh e foi confirmado ausente no teste local em modo production. `style-src` ainda mantém `'unsafe-inline'` temporariamente e é o próximo alvo de hardening.
...
- O nonce por requisição tornou as páginas server-rendered dinamicamente. Esse trade-off de cache/performance deve ser monitorado. Não houve afirmação de deploy em produção ou de enforcement ativo. Roubo de token ou sessão, cookies/refresh, sessões, rate limits e revisão OWASP ASVS continuam como hardening pré-beta; após `style-src`, permanece pendente avaliar futuramente uma CSP bloqueante.
