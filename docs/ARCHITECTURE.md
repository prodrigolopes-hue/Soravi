...
| Storage de rate limit | Redis compartilhado entre instâncias, sem fallback para memória (`d3f2183`). |
...
O commit `a36a8a1 test(auth): cobre concorrencia entre login e troca de senha` elevou a bateria a sete testes PostgreSQL reais e cobriu login versus troca autenticada da própria senha. Quando o login vence o lock, a troca espera de fato; o login cria uma sessão e comita, e a troca posterior altera o hash, preserva a `currentSession` e revoga a sessão concorrente, mantendo `lastLoginAt` atualizado. Quando a troca vence seu `SELECT ... FOR UPDATE`, o login pré-lê `ACTIVE` e o hash antigo, bloqueia no `user.update` e, após o commit do novo hash, falha com `InvalidCredentialsException`; nenhuma sessão nova persiste, `lastLoginAt` reverte para `null` e a `currentSession` continua ativa. A contenção de ambas as ordens é confirmada por `pg_backend_pid()` e `pg_blocking_pids()`.

A coordenação de teste usa Promises/barreiras e `setImmediate` somente para ceder o event loop no polling, sem `sleep`, `setTimeout` ou mocks de `$transaction`, `user.update` e `$queryRaw`; queries de `PasswordResetToken` e `AuthSession` também permanecem reais. Todos esses testes usam somente `soravi_integration_test`, sem dados do banco local normal `soravi` e sem migrations automáticas. A base continuará disponível para outros fluxos críticos ainda pendentes.
