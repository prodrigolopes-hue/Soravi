...
- [ ] Remover gradualmente o `'unsafe-inline'` temporário de `script-src` e `style-src` com nonce/hash; não liberar em produção o `'unsafe-eval'` observado apenas com Next.js/Fast Refresh local; somente depois avaliar CSP bloqueante. Revisar também XSS, proteção de sessão e token, cookies/refresh, rate limits e OWASP ASVS pré-beta.
