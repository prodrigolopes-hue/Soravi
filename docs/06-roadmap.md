...
O commit `9443580` concluiu também a cobertura integrada PostgreSQL real de login versus confirmação de password reset, nas duas ordens do lock e com contenção confirmada por `pg_backend_pid()` e `pg_blocking_pids()`. Permanecem pendentes login versus troca de senha autenticada, concorrência do limite máximo de sessões e outros cenários ainda não implementados. Isso não altera o estado documentado de ASVS, não declara conformidade geral e não afirma deploy.
