...
O bloco de moderação administrativa dos commits `fce91dc` e `7f7b5dd` concluiu bloqueio seguro, reativação, frontend `ACTIVE`/`BLOCKED` e revogação de sessões durante o bloqueio, sem afirmar deploy. ASVS 5.0.0 V7.4.2 permanece **parcialmente tratado**: ainda são necessários fluxos próprios de suspensão, desativação e exclusão/soft-delete que revoguem sessões explicitamente. O teste integrado real de concorrência PostgreSQL permanece no backlog.
